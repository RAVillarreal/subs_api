...
def get_google_link(query, site):
    ...
    url = 'https://www.google.com/search?client=ubuntu&channel=fs&q=site%3A{0}+{1}'.format(site, "+".join(query))
    response = requests.get(url)
    results = BeautifulSoup(response.content).find_all('div', attrs={"class": "g"})
    if results:
        best_match = [0, results[0]]
        for result in results:
            result_text = result.find("span", attrs={"class": "st"}).text.lower().split(" ")
            match_count = len(set(query).intersection(result_text))
            if match_count > best_match[0]:
                best_match[0] = match_count
                best_match[1] = result

        link = best_match[1].cite.text
        return link
...
def get_from_subdivx(query):
    ...
    link = get_google_link(query, 'subdivx.com')
    if link:
        response = requests.get(link)
        results = BeautifulSoup(response.content)
        code = re.search(r"(?P<Search>X5X)|(?P<Page>X6X)", link)
        if code:
            if code.group("Page"):
                download_link = results.find("a", attrs={"class": "link1"})["href"]
            elif code.group("Search"):
                download_link = results.find(
                    "div", attrs={"id": "buscador_detalle_sub_datos"}).find_all("a")[-1]["href"]
            else:
                return None
